  ...
  try {
    ...
    if (!name || !price || !category || !description) {
      return res.status(400).json({
        status: 400,
        message:
          "All fields (name, price, category, description) are required.",
      });
    }
...
app.post("/addtocart", authenticateToken, async (req, res) => {
  try {
    ...
    const isAvailable = await cartProductModel.findOne({
      productId: productId,
      userId: userId,
    });
  ...
  try {
    ...
    for (let order of pendingOrders) {
      if (order.buyerId.equals(userId)) {
        // Correct way to compare ObjectId
...
app.post("/deliveritems", authenticateToken, async (req, res) => {
  try {
    ...
    const pendingdelivers = await orderHistoryModel
      .find({ sellerId: userId, Delivered: "Pending" })
      .populate("prodId", "name price category description")
      .populate("buyerId", "firstName lastName email");
...
app.post("/completedeliver", authenticateToken, async (req, res) => {
  try {
    ...
    const pendingOrder = await orderHistoryModel.findOne({
      sellerId: userId,
      prodId: prodId,
    });
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    if (userMessage.includes("hello") || userMessage.includes("hi")) {
      return res.json(
        "Hello! Welcome to our platform. Are you looking to buy or sell something today? 🚀 Check out our amazing deals!"
      );
    }
    ...
    if (productKeywords.some((keyword) => userMessage.includes(keyword))) {
      return res.json(
        "We have a variety of amazing products available! 🛍️ Check them out on our [Store Page](http://localhost:5173/searchitems) for the latest listings."
      );
    }
    const sellkeywords = [
      "sell",
    ...
    if (sellkeywords.some((keyword) => userMessage.includes(keyword))) {
      return res.json(
        "You Can go to the [Store Page](http://localhost:5173/searchitems) and upload your items overthere"
      );
    }
